...
new #[Title('إدارة طرق الدفع')] class extends Component {
    use WithPagination;
        ...
        return PaymentMethod::query()
            ...
            ->when($this->search, function ($q) {
                $q->where(function ($query) {
                    $query->where('payment_method_name', 'like', "%{$this->search}%");
                });
            })
            ->paginate(10);
    }

    public function getTotalPaymentMethodsProperty()
    {
    ...
    public function toggleActive($id)
    {
        ...
        $status = $paymentMethod->is_active ? 'نشط' : 'غير نشط';
        session()->flash('success', "تم تغيير حالة طريقة الدفع إلى {$status} بنجاح");
    }
};
?>
...
<div dir="rtl"
    ...
    <div class="mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            <div class="space-y-1">
                <div class="flex items-center gap-3">
                    ...
                    <span
                        ...
                        {{ $this->totalPaymentMethods }} إجمالي
                    </span>
                </div>
                <p class="text-sm text-gray-500 dark:text-gray-400 pr-5">
                    إدارة وتنظيم جميع طرق الدفع بكفاءة عالية وإحصائيات دقيقة
                </p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-all duration-300"
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1.5 mr-2.5">
                            عرض {{ $this->payment_methods->firstItem() ?? 0 }} -
                            {{ $this->payment_methods->lastItem() ?? 0 }}
                            من أصل <span
                                class="font-medium text-gray-600 dark:text-gray-300">{{ $this->payment_methods->total() }}</span>
                            طريقة دفع
                        </p>
                    </div>
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="بحث في طرق الدفع..."
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 transition"
                                wire:click="sortByColumn('payment_method_name')">
                                <div class="flex items-center gap-1">
                                    اسم طريقة الدفع
                                    @if ($sortBy === 'payment_method_name')
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}">
                                            </path>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800/50">
                        @forelse ($this->payment_methods as $payment_method)
                            <tr wire:key="payment_method-{{ $payment_method->id }}"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            {{ mb_substr($payment_method->payment_method_name, 0, 2) }}
                                        </div>
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 font-mono">
                                                كود طريقة الدفع: {{ $payment_method->id }}
                                            </p>
                                        </div>
                                    </div>
                                 </td>

                                {{-- is_active column with toggle and badge --}}
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        {{-- Badge --}}
                                        <span
                                            class="px-2.5 py-1 text-xs font-medium rounded-full {{ $payment_method->is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' }}">
                                            {{ $payment_method->is_active ? 'نشط' : 'غير نشط' }}
                                        </span>
                                        {{-- Toggle Switch --}}
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-0.5">
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                            {{ $payment_method->created_at->translatedFormat('d F Y') }} </span>
                                        <span class="text-xs text-gray-400 dark:text-gray-500">
                                            {{ $payment_method->created_at->diffForHumans() }}
                                        </span>
                                    </div>
                                 </td>
                                    ...
                                    <div class="flex flex-col items-center gap-4">
                                        ...
                                        <div class="text-center">
                                            <p class="text-base font-semibold text-gray-700 dark:text-gray-300">لا توجد
                                                طرق دفع</p>
                                            <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">ابدأ بإضافة طريقة دفع
            ...
            @if ($this->payment_methods->hasPages())
                <div
