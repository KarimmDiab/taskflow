...
new #[Title('كولكشن')] class extends Component {
    use WithPagination;
        ...
        return Collection::query()
            ...
            ->when($this->search, function ($q) {
                $q->where(function ($query) {
                    $query->where('collection_name', 'like', "%{$this->search}%")->orWhere('color_hex_code', 'like', "%{$this->search}%");
                });
            })
    ...
    {
        abort_unless(auth()->user()?->can('collections.update'), 403);

        $this->dispatch('editCollection', id: $id);
    }

    public function delete($id)
    {
        abort_unless(auth()->user()?->can('collections.delete'), 403);

        Collection::findOrFail($id)->delete();
        session()->flash('success', 'تم حذف الكولكشن بنجاح');
        $this->resetPage();
    }
...
<div dir="rtl"
    ...
    <div class="mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    منصة متكاملة لإدارة وتنظيم جميع الكولكشن بكفاءة عالية
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-blue-600 dark:text-blue-400">إجمالي الالوان</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">
                                    {{ $this->totalCollections }}</p>
                            </div>
                            <div
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            @php
                                $lastUpdated = Collection::latest('updated_at')->value('updated_at');
                            @endphp
        ...
        {{-- Modals --}}
        @can('collections.create') <livewire:collection.create /> @endcan

        @can('collections.update') <livewire:collection.edit /> @endcan
                ...
                <div class="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1.5 mr-3">
                            عرض {{ $this->collections->firstItem() ?? 0 }} - {{ $this->collections->lastItem() ?? 0 }}
                            من أصل <span
                                class="font-semibold text-gray-600 dark:text-gray-300">{{ $this->collections->total() }}</span>
                            لون
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <select wire:model.live="perPage"
                            ...
                            <option value="10">10 كولكشن</option>
                            <option value="25">25 كولكشن</option>
                            <option value="50">50 كولكشن</option>
                            <option value="100">100 كولكشن</option>
                        </select>
                    </div>
                </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50/80 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 transition group"
                                wire:click="sortByColumn('collection_name')">
                                <div class="flex items-center gap-1">
                                    اسم الكولكشن
                                    @if ($sortBy === 'collection_name')
                        ...
                        @forelse ($this->collections as $collection)
                            <tr wire:key="collection-{{ $collection->id }}"
                                ...
                                <td class="px-6 py-4 text-gray-400 dark:text-gray-600 text-xs font-mono font-medium">
                                    {{ $loop->iteration + ($this->collections->currentPage() - 1) * $this->collections->perPage() }}
                                </td>

                                <!-- Collection name with image or first 2 letters -->
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        @if ($collection->main_image)
                                            <img src="{{ Storage::url($collection->main_image) }}"
                                        ...
                                        @else
                                            <div
                                                ...
                                                {{ mb_substr($collection->collection_name, 0, 2) }}
                                            </div>
                                        @endif
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-0.5">
                                        <span
                                            class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $collection->created_at->translatedFormat('d F Y') }}</span>
                                        <span
                                            class="text-xs text-gray-400 dark:text-gray-500">{{ $collection->created_at->diffForHumans() }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end items-center gap-2">
                                        ...
                                        <button wire:click="delete({{ $collection->id }})"
                                            wire:confirm="هل أنت متأكد من حذف هذا الكولكشن ؟"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-20 text-center">
                                    ...
                                    <div class="text-gray-400 dark:text-gray-500">لا توجد مجموعات</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
