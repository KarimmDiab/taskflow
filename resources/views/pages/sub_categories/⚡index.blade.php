...
new #[Title('إدارة التصنيفات الفرعية للمنتجات')] class extends Component {
    use WithPagination;
    ...
    {
        abort_unless(auth()->user()?->can('sub_categories.update'), 403);

        $this->dispatch('editSubCategory', id: $id);
    }

    public function delete($id)
    {
        abort_unless(auth()->user()?->can('sub_categories.delete'), 403);
...
<div dir="rtl"
    ...
    <div class="mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {{-- Modals --}}
        @can('sub_categories.create') <livewire:sub-category.create /> @endcan
        @can('sub_categories.update') <livewire:sub-category.edit /> @endcan
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                    <div>
                        <h3 class="text-base font-semibold text-gray-800 dark:text-white flex items-center gap-2">
                            ...
                            قائمة التصنيفات الفرعية للمنتجات
                        </h3>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1.5 mr-2.5">
                            عرض {{ $this->SubCategory->firstItem() ?? 0 }} -
                            {{ $this->SubCategory->lastItem() ?? 0 }}
                            من أصل <span
                                class="font-medium text-gray-600 dark:text-gray-300">{{ $this->SubCategory->total() }}</span>
                            تصنيف فرعي
                        </p>
                    </div>
                    <div class="relative">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300 transition"
                                wire:click="sortByColumn('sub_category_name')">
                                <div class="flex items-center gap-1">
                                    اسم التصنيف الفرعي
                                    @if ($sortBy === 'sub_category_name')
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}">
                                            </path>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800/50">
                        @forelse ($this->SubCategory as $subCategory)
                            <tr wire:key="subCategory-{{ $subCategory->id }}"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            {{ mb_substr($subCategory->sub_category_name, 0, 2) }}
                                        </div>
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 font-mono">
                                                كود التصنيف الفرعي: {{ $subCategory->id }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                                        ...
                                        <span
                                            class="text-sm truncate max-w-[220px]">{{ $subCategory->category->category_name }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                                        ...
                                        <span
                                            class="text-sm truncate max-w-[220px]">{{ $subCategory->sub_category_description }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-0.5">
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                            {{ $subCategory->created_at->translatedFormat('d F Y') }} </span>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end items-center gap-2">
                                        ...
                                        <button wire:click="delete({{ $subCategory->id }})"
                                            wire:confirm="هل أنت متأكد من حذف هذا التصنيف الفرعي ؟"
                                    ...
                                    <div class="flex flex-col items-center gap-4">
                                        ...
                                        <div class="text-center">
                                            <p class="text-base font-semibold text-gray-700 dark:text-gray-300">لا توجد
                                                تصنيفات فرعية</p>
                                            <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">ابدأ بإضافة تصنيف فرعي
