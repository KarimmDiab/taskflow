...
new class extends Component {
    ...
    public bool $isUpdating = false; // fixed event name
    ...
    public function updateSize()
    {
        ...
        try {
            ...
            session()->flash('warning', 'تم تحديث بيانات المقاس بنجاح');
            $this->redirectRoute('sizes', navigate: true); // fixed: was 'size'
        } catch (\Exception $e) {
            session()->flash('error', 'حدث خطأ أثناء تحديث المقاس');
    ...
    public function confirmDelete($id)
    {
        $size = Size::findOrFail($id);
        $this->form->setSize($size);
        Flux::modal('delete-size')->show();
    }

    public function deleteSize()
    {
        $this->form->size->delete();
        Flux::modal('delete-size')->close();
        session()->flash('success', 'تم حذف بيانات المقاس بنجاح');
        $this->redirectRoute('sizes', navigate: true);
    }
};
?>
...
<div>
    ...
    <flux:modal name="edit-size" class="md:w-[600px] overflow-hidden rounded-2xl" style="padding: 0;">

        <form wire:submit.prevent="updateSize" class="relative">
            ...
            {{-- Header --}}
            <div
                class="bg-gradient-to-r from-gray-50 to-white dark:from-zinc-800 dark:to-zinc-800/90 px-6 py-5 border-b border-gray-200 dark:border-zinc-700">
                <div class="flex items-center gap-3">
                    <div
                        class="p-2.5 bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/30 dark:to-orange-900/30 rounded-xl shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-amber-600 dark:text-amber-400"
            ...
            <div class="p-6 space-y-6 bg-white dark:bg-zinc-900">
                ...
                <div>
                    <flux:label class="mb-1.5 font-semibold text-gray-700 dark:text-gray-300">
                        اسم المقاس <span class="text-red-500 dark:text-red-400">*</span>
                    </flux:label>
                    <flux:input placeholder="مثال: XXL" wire:model="form.size_name"
                ...
                <div>
                    <flux:label class="mb-1.5 font-semibold text-gray-700 dark:text-gray-300">
                        ترتيب العرض <span class="text-red-500 dark:text-red-400">*</span>
                    </flux:label>
                    <flux:input type="number" min="0" step="1" placeholder="مثال: 1"
                        wire:model="form.sort_order" wire:dirty.class="border-amber-400 ring-2 ring-amber-400/50"
                        class="w-full px-4 py-2.5 rounded-xl border-gray-300 dark:border-zinc-600 focus:border-amber-500 focus:ring-amber-500 transition-all duration-200"
                        style="border-width: 1px;" />
                    <flux:text class="text-xs text-gray-400 mt-1">الأرقام الأصغر تظهر أولاً في القوائم</flux:text>
                ...
                <div
                    class="flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-700">
                    <div>
                        <flux:label class="font-semibold text-gray-700 dark:text-gray-300">
                            حالة المقاس
                        </flux:label>
                        <flux:text class="text-xs text-gray-500 dark:text-gray-400">
                            تعطيل المقاس يجعله غير متاح للاختيار في النظام
                        </flux:text>
                    </div>
                    <div class="relative">
                        <input type="checkbox" wire:model="form.is_active" id="is_active_toggle_edit"
                            class="sr-only peer">
