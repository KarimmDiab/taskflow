...
new class extends Component {
    ...
    public function save()
    {
        ...
        try {
            $this->form->store();
            Flux::modal('add-gov')->close();
            session()->flash('success', 'تم اضافة المحافظة بنجاح');
            $this->redirectRoute('shipping', navigate: true);
        } catch (\Exception $e) {
            session()->flash('error', 'حدث خطأ أثناء اضافة المحافظة');
            $this->isSaving = false;
        }
    }
};
...
<div>
    ...
    <flux:modal name="add-gov" class="md:w-[600px] overflow-hidden rounded-2xl" style="padding: 0;">
        ...
        <form wire:submit.prevent="save" class="relative">
            ...
            <div
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="p-2.5 bg-gradient-to-br from-emerald-100 to-teal-100 dark:from-emerald-900/30 dark:to-teal-900/30 rounded-xl shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-emerald-600 dark:text-emerald-400"
                    ...
                    <div>
                        ...
                        <flux:text class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            قم بإدخال تفاصيل المحافظة لإضافته إلى النظام
                        </flux:text>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 space-y-6 bg-white dark:bg-zinc-900">
                ...
                <div>
                    <flux:label class="mb-1.5 font-semibold text-gray-700 dark:text-gray-300">
                        اسم المحافظة <span class="text-red-500 dark:text-red-400">*</span>
                    </flux:label>
                    <flux:input placeholder="مثال: القاهرة" wire:model="form.city_name"
                ...
                <div>
                    <flux:label class="mb-1.5 font-semibold text-gray-700 dark:text-gray-300">
                        تكلفة الشحن (ج.م) <span class="text-red-500 dark:text-red-400">*</span>
                    </flux:label>
                    <flux:input type="number" step="0.01" placeholder="مثال: 50.00" wire:model="form.shipping_cost"
                        class="w-full px-4 py-2.5 rounded-xl border-gray-300 dark:border-zinc-600 focus:border-emerald-500 focus:ring-emerald-500 transition-all duration-200"
                        style="border-width: 1px;" />
                ...
                <div class="flex items-center justify-between">
                    ...
                    <flux:switch wire:model="form.is_active" label="{{ $form->is_active ? 'نشط' : 'غير نشط' }}"
                        class="peer-checked:bg-emerald-500" />
                </div>
                <!-- Branch Address -->
                <div>
                    <flux:label class="mb-1.5 font-semibold text-gray-700 dark:text-gray-300">
                        الوقت المتوقع <span class="text-red-500 dark:text-red-400">*</span>
                    </flux:label>
                    <flux:textarea placeholder="مثال: 2 - 3 ايام" wire:model="form.estimated_days" rows="3"
