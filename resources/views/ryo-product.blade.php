...
<div style="max-width:1440px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:0;" id="pdpLayout">
    ...
    <div style="padding:32px 40px 0 40px;display:flex;gap:16px;position:sticky;top:64px;align-self:start;max-height:calc(100vh - 64px);">
        <div style="display:flex;flex-direction:column;gap:8px;flex-shrink:0;" id="thumbs">
            {{-- سيتم ملء thumbs عبر JavaScript --}}
        </div>
        <div class="main-img-wrap" style="flex:1;cursor:zoom-in;" onclick="window.lightbox?.open()">
            <img id="mainImg" src="" alt="{{ $selectedProdcut->product_name }}">
            <div class="zoom-badge"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>Click to Zoom</div>
            <button class="md:hidden" onclick="window.gallery.prev(event)" style="position:absolute;left:12px;top:50%;transform:translateY(-50%);background:rgba(248,246,242,.85);border:none;cursor:pointer;width:36px;height:36px;display:flex;align-items:center;justify-content:center;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M15 18l-6-6 6-6"/></svg></button>
            <button class="md:hidden" onclick="window.gallery.next(event)" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:rgba(248,246,242,.85);border:none;cursor:pointer;width:36px;height:36px;display:flex;align-items:center;justify-content:center;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 18l6-6-6-6"/></svg></button>
        </div>
    </div>
    ...
    <div style="padding:40px 40px 80px 48px;max-width:560px;" id="productInfo">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
            <p style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:.2em;text-transform:uppercase;color:#9C9A96;">
                RYO · {{ $selectedProdcut->slug }}
            </p>
        </div>
        ...
        <div style="display:flex;align-items:baseline;gap:16px;margin-bottom:32px;" id="priceBlock">
            <span id="currentPrice" style="font-family:'DM Sans',sans-serif;font-size:22px;font-weight:400;color:#0A0A0A;">EGP {{ number_format($price) }}</span>
            <span id="oldPrice" style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:.15em;text-transform:uppercase;color:#9C9A96;text-decoration:line-through;">EGP {{ number_format($oldPrice) }}</span>
            <span style="background:#0A0A0A;color:white;padding:3px 6px;border-radius:999px;font-family:'Space Grotesk',sans-serif;font-size:7px;letter-spacing:.12em;text-transform:uppercase;">Discount 25%</span>
        </div>

        <div style="height:1px;background:black;margin-bottom:28px;"></div>

        @php
            $variantsJson = $selectedProdcut->productVariants->map(fn($v) => [
                'id' => $v->id,
                'colorId' => $v->color_id,
                'colorName' => $v->color?->color_name ?? '',
                'colorHex' => $v->color?->color_hex_code ?? '#000',
                'sizeId' => $v->size_id,
                'sizeName' => $v->size?->size_name ?? '',
                'price' => (float)$v->variant_price,
                'stock' => (int)$v->inventories->sum('quantity'),
                'sku' => $v->sku ?? ''
            ])->values();
        @endphp
        ...
        <script>
            ...
            // تجهيز بيانات الصور حسب اللون (color_id)
            window.productImages = {
                @php
                    $imagesByColor = [];
                    foreach ($selectedProdcut->images as $image) {
                        $colorId = $image->color_id ?? 'default';
                        $imagesByColor[$colorId][] = Storage::url($image->image_path);
                    }
                @endphp
        ...
        <div style="margin-bottom:28px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                <p style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:.18em;text-transform:uppercase;">
                    Color: <span id="selectedColorLabel" style="color:#9C9A96;font-weight:400;">{{ $firstVariant?->color?->color_name ?? '' }}</span>
                </p>
            </div>
            <div style="display:flex;gap:10px;align-items:center;" id="colorSwatches">
                @php $uniqueColors = $selectedProdcut->productVariants->unique('color_id'); @endphp
                @foreach ($uniqueColors as $variant)
                    <div class="color-swatch-opt {{ $loop->first ? 'active' : '' }}"
                         ...
                         title="{{ $variant->color?->color_name }}"
                         onclick="selectColor(this, {{ $variant->color_id }})">
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div style="margin-bottom:32px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                <p style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:.18em;text-transform:uppercase;">
                    Size: <span id="selectedSizeLabel" style="color:#9C9A96;font-weight:400;">— Select a size</span>
                </p>
                <button onclick="window.sizeGuide.open()" style="background:none;border:none;cursor:pointer;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;text-decoration:underline;text-underline-offset:3px;display:flex;align-items:center;gap:5px;">
                    Size Guide<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                </button>
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;" id="sizeBtns"></div>
            <p id="sizeError" style="font-family:'DM Sans',sans-serif;font-size:11px;color:#C0392B;margin-top:8px;display:none;">Please select a size to continue.</p>
        </div>
        <p id="stockBadge" style="font-family:'DM Sans',sans-serif;font-size:11px;color:#9C9A96;margin-top:-20px;margin-bottom:20px;display:none;"></p>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0;border:1px solid #EDEDEB;margin-bottom:32px;">
            <div style="padding:14px 12px;text-align:center;border-right:1px solid #EDEDEB;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9C9A96" stroke-width="1.5" style="margin:0 auto 6px;display:block;"><path d="M5 12h14"/><path d="M12 5l7 7-7 7"/></svg><p style="font-family:'Space Grotesk',sans-serif;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;line-height:1.4;">Free Delivery<br>EGP 2,000+</p></div>
            <div style="padding:14px 12px;text-align:center;border-right:1px solid #EDEDEB;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9C9A96" stroke-width="1.5" style="margin:0 auto 6px;display:block;"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 014-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 01-4 4H3"/></svg><p style="font-family:'Space Grotesk',sans-serif;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;line-height:1.4;">14-Day<br>Returns</p></div>
            <div style="padding:14px 12px;text-align:center;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9C9A96" stroke-width="1.5" style="margin:0 auto 6px;display:block;"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg><p style="font-family:'Space Grotesk',sans-serif;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;line-height:1.4;">Secure<br>Checkout</p></div>
        </div>
        ...
        <div style="margin-bottom:0;">
            <div class="accordion-item open" id="acc-desc">
                <div class="accordion-header">Description<svg class="accordion-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
                <div class="accordion-body"><div class="accordion-content"><p style="margin-bottom:12px;">The RYO Essential Tee is our signature oversized silhouette — designed for those who understand that less is more. Cut from 350gsm heavyweight cotton, this tee drapes with intention and improves with every wash.</p><p>Dropped shoulders, a relaxed chest, and a slightly elongated hem create a premium casual look that works from morning to midnight.</p></div></div>
            </div>
            <div class="accordion-item" id="acc-details">
                <div class="accordion-header">Material & Construction<svg class="accordion-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
                <div class="accordion-body"><div class="accordion-content"><ul><li>100% Premium Egyptian Cotton, 350gsm</li><li>Heavyweight, garment-dyed finish</li><li>Dropped shoulder, relaxed silhouette</li><li>Reinforced double-stitched seams</li><li>Ribbed collar with slight stretch</li><li>Elongated back hem</li><li>Preshrunk — minimal further shrinkage</li></ul></div></div>
            </div>
            <div class="accordion-item" id="acc-fit">
                <div class="accordion-header">Fit & Sizing<svg class="accordion-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
                <div class="accordion-body"><div class="accordion-content"><p style="margin-bottom:12px;">Fits oversized. Model is 185cm and wears size M. For a relaxed but not overwhelming fit, we recommend sizing down.</p><table style="width:100%;border-collapse:collapse;"><thead><tr style="border-bottom:1px solid #EDEDEB;"><th style="font-family:'Space Grotesk',sans-serif;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;text-align:left;padding:8px 0;">Size</th><th style="font-family:'Space Grotesk',sans-serif;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;text-align:left;padding:8px 0;">Chest</th><th style="font-family:'Space Grotesk',sans-serif;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;text-align:left;padding:8px 0;">Length</th></tr></thead><tbody><tr><td style="padding:8px 0;">XS</td><td style="padding:8px 0;color:#9C9A96;">108cm</td><td style="padding:8px 0;color:#9C9A96;">70cm</td></tr><tr><td style="padding:8px 0;">S</td><td style="padding:8px 0;color:#9C9A96;">114cm</td><td style="padding:8px 0;color:#9C9A96;">72cm</td></tr><tr><td style="padding:8px 0;">M</td><td style="padding:8px 0;color:#9C9A96;">120cm</td><td style="padding:8px 0;color:#9C9A96;">74cm</td></tr><tr><td style="padding:8px 0;">L</td><td style="padding:8px 0;color:#9C9A96;">126cm</td><td style="padding:8px 0;color:#9C9A96;">76cm</td></tr><tr><td style="padding:8px 0;">XL</td><td style="padding:8px 0;color:#9C9A96;">132cm</td><td style="padding:8px 0;color:#9C9A96;">78cm</td></tr></tbody></table></div></div>
            </div>
            <div class="accordion-item" id="acc-care">
                <div class="accordion-header">Care Instructions<svg class="accordion-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
                <div class="accordion-body"><div class="accordion-content"><ul><li>Machine wash cold, gentle cycle</li><li>Do not bleach</li><li>Tumble dry low heat</li><li>Cool iron if needed</li><li>Do not dry clean</li><li>Turn inside out to preserve colour</li></ul></div></div>
            </div>
            <div class="accordion-item" id="acc-shipping" style="border-bottom:none;">
                <div class="accordion-header">Shipping & Returns<svg class="accordion-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
                <div class="accordion-body"><div class="accordion-content"><ul><li>Standard delivery: 2–4 business days</li><li>Free delivery on orders over EGP 2,000</li><li>Express delivery available at checkout</li><li>Free returns within 14 days of delivery</li><li>Items must be unworn with tags attached</li></ul></div></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function setMainImage(index) {
        if (window.currentImages && window.currentImages[index]) {
            document.getElementById('mainImg').src = window.currentImages[index];
            window.currentImageIndex = index;
            // تحديث الـ active class على الـ thumbnails
    ...
    function updateGalleryByColor(colorId) {
        let images = window.productImages[colorId] || window.productImages['default'] || [];
        if (images.length === 0) {
        ...
        thumbsContainer.innerHTML = images.map((url, idx) => `
            <div class="gallery-thumb ${idx === 0 ? 'active' : ''}" data-img-index="${idx}" onclick="setMainImage(${idx})">
    ...
    function selectColor(element, colorId) {
        ...
        document.querySelectorAll('.color-swatch-opt').forEach(sw => sw.classList.remove('active'));
        element.classList.add('active');
        // تحديث النص المعروض للون
        ...
        // (اختياري) تحديث المقاسات والأسعار حسب اللون
        updateSizesAndPriceByColor(colorId);
    }

    function updateSizesAndPriceByColor(colorId) {
        const variants = window.VARIANTS.filter(v => v.colorId == colorId);
        const sizeContainer = document.getElementById('sizeBtns');
        if (!sizeContainer) return;
        if (variants.length === 0) {
            sizeContainer.innerHTML = '<span style="color:#9C9A96;">No sizes available</span>';
            return;
        }
        // عرض أزرار المقاسات
        sizeContainer.innerHTML = variants.map(v => `
            <button class="size-opt" data-size-id="${v.sizeId}" data-price="${v.price}" data-stock="${v.stock}" data-variant-id="${v.id}" onclick="selectSize(this, ${v.sizeId}, ${v.price})">
                ${v.sizeName}
            </button>
        `).join('');
        // تحديث السعر لآخر مقاس مفعل (اختياري)
        if (variants.length > 0) {
            document.getElementById('currentPrice').innerText = 'EGP ' + variants[0].price.toLocaleString();
        }
    }

    function selectSize(element, sizeId, price) {
        // إزالة active من جميع المقاسات
        document.querySelectorAll('.size-opt').forEach(btn => btn.classList.remove('active'));
        element.classList.add('active');
        document.getElementById('selectedSizeLabel').innerText = element.innerText;
        document.getElementById('currentPrice').innerText = 'EGP ' + price.toLocaleString();
        // تخزين الـ variant المختار عالمياً
    ...
    window.gallery = {
        prev: (event) => {
            ...
            if (window.currentImages && window.currentImages.length) {
                let newIndex = (window.currentImageIndex - 1 + window.currentImages.length) % window.currentImages.length;
        ...
        next: (event) => {
            event?.stopPropagation();
            if (window.currentImages && window.currentImages.length) {
                let newIndex = (window.currentImageIndex + 1) % window.currentImages.length;
                setMainImage(newIndex);
            }
        }
    };

    // عند تحميل الصفحة، قم بتحديث المعرض بناءً على اللون النشط
    document.addEventListener('DOMContentLoaded', () => {
        const activeSwatch = document.querySelector('.color-swatch-opt.active');
        if (activeSwatch) {
            const colorId = activeSwatch.getAttribute('data-color-id');
            updateGalleryByColor(colorId);
            updateSizesAndPriceByColor(colorId);
        } else {
