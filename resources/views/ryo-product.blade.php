    ...
    <div style="max-width:1440px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:0;" id="pdpLayout">
        ...
        <div
            ...
            <div class="main-img-wrap" style="flex:1;cursor:zoom-in;" onclick="window.lightbox?.open()">
                ...
                <button type="button" data-gallery-nav="prev" onclick="window.gallery.prev(event)"
                    style="position:absolute;left:12px;top:50%;transform:translateY(-50%);background:rgba(248,246,242,.85);border:1px solid rgba(10,10,10,.08);cursor:pointer;width:36px;height:36px;display:flex;align-items:center;justify-content:center;"><svg
                        width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="1.5">
                        <path d="M15 18l-6-6 6-6" />
                    </svg></button>
                <button type="button" data-gallery-nav="next" onclick="window.gallery.next(event)"
                    style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:rgba(248,246,242,.85);border:1px solid rgba(10,10,10,.08);cursor:pointer;width:36px;height:36px;display:flex;align-items:center;justify-content:center;"><svg
                        width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="1.5">
        ...
        <div style="padding:40px 40px 80px 48px;max-width:560px;" id="productInfo">
            ...
            @php
                $variantsJson = $selectedProdcut->productVariants
                    ->map(
                        fn($v) => [
                            'id' => $v->id,
                            'colorId' => $v->color_id,
            ...
            <script id="product-data" type="application/json">
{
    ...
    "productImages": {
        "all": @json($allImageUrls),
        "byColor": {
            @foreach ($imagesByColor as $colorId => $urls)
                "{{ $colorId }}": @json($urls)@if (! $loop->last),@endif
            @endforeach
        },
        "placeholderImage": @json(asset('images/placeholder.jpg'))
    }
}
        </script>
                ...
                <div style="display:flex;gap:10px;align-items:center;" id="colorSwatches">
                    ...
                    @foreach ($uniqueColors as $variant)
                        <div class="color-swatch-opt {{ $loop->first ? 'active' : '' }}"
                            ...
                            onclick="window.selectColor(this, @json($variant->color_id))">
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div style="margin-bottom:32px;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                    ...
                    <button onclick="window.sizeGuide.open()"
                        style="background:none;border:none;cursor:pointer;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:#9C9A96;text-decoration:underline;text-underline-offset:3px;display:flex;align-items:center;gap:5px;">
                        Size Guide<svg width="11" height="11" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="1.5">
    ...
    <script src="{{ asset('js/ryo-product-page.js') }}" defer></script>
