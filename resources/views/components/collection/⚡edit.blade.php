...
new class extends Component
{
    ...
    public function editCollection($id)
    {
        ...
        Flux::modal('edit-collection')->show();
    }
    ...
    public function updateCollection()
    {
        ...
        try {
            ...
            session()->flash('success', 'تم تحديث بيانات الكولكشن بنجاح');
            $this->redirectRoute('all_collections', navigate: true);
        } catch (\Exception $e) {
            session()->flash('error', 'حدث خطأ أثناء تحديث الكولكشن');
    ...
    public function confirmDelete($id)
    {
        $collection = Collection::findOrFail($id);
        $this->form->setCollection($collection);
        Flux::modal('delete-collection')->show();
    }

    public function deleteCollection()
    {
        // Optional: delete image from storage before deleting record
        if ($this->form->collection->main_image && Storage::disk('public')->exists($this->form->collection->main_image)) {
            Storage::disk('public')->delete($this->form->collection->main_image);
        }
        $this->form->collection->delete();
        Flux::modal('delete-collection')->close();
        session()->flash('danger', 'تم حذف الكولكشن بنجاح');
        $this->redirectRoute('all_collections', navigate: true);
    }
};
?>
...
<flux:modal name="edit-collection" class="md:w-[600px] overflow-hidden rounded-2xl" style="padding: 0;">

    <form wire:submit.prevent="updateCollection" enctype="multipart/form-data" class="relative">
        ...
        <div class="p-6 space-y-6 bg-white dark:bg-zinc-900">
            ...
            <div>
                ...
                <flux:input
                    ...
                    wire:dirty.class="border-emerald-400 ring-2 ring-emerald-400/50"
