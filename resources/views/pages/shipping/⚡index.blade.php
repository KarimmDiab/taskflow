...
new #[Title('مقاسات التيشرتات')] class extends Component {
    use WithPagination;
            ...
            ->when($this->search, function ($q) {
                $q->where(function ($query) {
                    $query->where('city_name', 'like', "%{$this->search}%")
                          ...
                          ->orWhere('expected_delivery', 'like', "%{$this->search}%");
                });
            })
    ...
    public function getTotalShippingsProperty()
    {
    ...
    {
        abort_unless(auth()->user()?->can('shipping_governorates.update'), 403);

        $this->dispatch('editShipping', id: $id);
    }

    public function delete($id)
    {
        abort_unless(auth()->user()?->can('shipping_governorates.delete'), 403);
...
<div dir="rtl"
    ...
    <div class="mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div
            ...
            <div
                class="absolute inset-0 bg-gradient-to-r from-blue-600/10 to-indigo-600/10 dark:from-blue-600/5 dark:to-indigo-600/5">
            </div>
            <div class="absolute top-0 right-0 w-64 h-64 bg-blue-400/10 rounded-full filter blur-3xl"></div>
            <div class="absolute bottom-0 left-0 w-64 h-64 bg-indigo-400/10 rounded-full filter blur-3xl"></div>
            ...
            <div class="relative px-6 py-8">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    منصة متكاملة لإدارة وتنظيم جميع تكاليف الشحن بكفاءة عالية
                                </p>
                            </div>
                        </div>
                    </div>

                    @can('shipping_governorates.create')
                    <flux:modal.trigger name="add-gov">
                        <button
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                    <div
                        class="bg-gradient-to-br from-blue-50 to-white dark:from-blue-950/20 dark:to-gray-900 rounded-xl p-4 border border-blue-100 dark:border-blue-900/30">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-blue-600 dark:text-blue-400">إجمالي المحافظات</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">
                                    {{ $this->totalShippings }}</p>
                            </div>
                            <div
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            @php
                                $lastUpdated = Shipping::latest('updated_at')->value('updated_at');
                            @endphp

                            <div>
                                <p class="text-sm text-purple-600 dark:text-purple-400">آخر تحديث</p>
        ...
        {{-- Modals --}}
        @can('shipping_governorates.create') <livewire:shipping.create /> @endcan
        @can('shipping_governorates.update') <livewire:shipping.edit /> @endcan
                ...
                <div class="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1.5 mr-3">
                            عرض {{ $this->shippings->firstItem() ?? 0 }} - {{ $this->shippings->lastItem() ?? 0 }}
                            من أصل <span
                                class="font-semibold text-gray-600 dark:text-gray-300">{{ $this->shippings->total() }}</span>
                            محافظة
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <div class="relative">
                            ...
                            <input type="text" wire:model.live.debounce.300ms="search"
                                ...
                                class="w-full sm:w-80 text-sm border border-gray-200 dark:border-gray-700 rounded-xl pr-9 pl-4 py-2.5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                        </div>

                        <select wire:model.live="perPage"
                            class="text-sm border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                            <option value="10">10 محافظات</option>
                            <option value="25">25 محافظة</option>
                            <option value="50">50 محافظة</option>
                            <option value="100">100 محافظة</option>
                        </select>
                    </div>
                </div>
                ...
                @if ($showFilters)
                    <div class="mt-5 pt-5 border-t border-gray-100 dark:border-gray-800">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">من
                                    تاريخ</label>
                                <input type="date" wire:model.live="dateFrom"
                                    class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">إلى
                                    تاريخ</label>
                        ...
                        <div class="mt-4 flex justify-end">
                            <button wire:click="resetFilters"
                                class="px-4 py-2 text-sm font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 transition-colors">
                                إعادة تعيين الفلاتر
                            </button>
                        </div>
                    </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50/80 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 transition group"
                                wire:click="sortByColumn('city_name')">
                                <div class="flex items-center gap-1">
                                    اسم المحافظة
                                    @if ($sortBy === 'city_name')
                            ...
                            {{-- NEW COLUMN: تكلفة الشحن --}}
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 transition group"
                                wire:click="sortByColumn('shipping_cost')">
                                <div class="flex items-center gap-1">
                                    تكلفة الشحن
                            ...
                            {{-- NEW COLUMN: الوقت المتوقع --}}
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 transition group"
                                wire:click="sortByColumn('expected_delivery')">
                                <div class="flex items-center gap-1">
                                    الوقت المتوقع
                                    @if ($sortBy === 'expected_delivery')
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}">
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800/50">
                        @forelse ($this->shippings as $shipping)
                            <tr wire:key="shipping-{{ $shipping->id }}"
                                class="hover:bg-gradient-to-r hover:from-blue-50/40 hover:to-transparent dark:hover:from-blue-900/10 transition-all duration-300 group">
                                <td class="px-6 py-4 text-gray-400 dark:text-gray-600 text-xs font-mono font-medium">
                                    {{ $loop->iteration + ($this->shippings->currentPage() - 1) * $this->shippings->perPage() }}
                                 </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/40 dark:to-indigo-900/40 flex items-center justify-center text-sm font-bold text-blue-700 dark:text-blue-300 shadow-sm group-hover:scale-105 transition-transform">
                                            {{ mb_substr($shipping->city_name, 0, 2) }}
                                        </div>
                                        <div>
                                ...
                                {{-- NEW CELL: تكلفة الشحن --}}
                                <td class="px-6 py-4">
                                    <div class="text-sm font-semibold text-gray-900 dark:text-white">
                                        {{ number_format($shipping->shipping_cost) }} ج.م
                                    </div>
                                 </td>
                                {{-- NEW CELL: الوقت المتوقع --}}
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-700 dark:text-gray-300">
                                        {{ $shipping->estimated_days ?? 'غير محدد' }}
                                    </div>
                                 </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-0.5">
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                            {{ $shipping->created_at->translatedFormat('d F Y') }}
                                        </span>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end items-center gap-2">
                                        @can('shipping_governorates.update')
                                        ...
                                        @endcan
                                        @can('shipping_governorates.delete')
                                        <button wire:click="delete({{ $shipping->id }})"
                                            wire:confirm="هل أنت متأكد من حذف هذه المحافظة؟"
                                    ...
                                    <div class="flex flex-col items-center gap-4">
                                        ...
                                        <div class="text-center">
                                            <p class="text-lg font-semibold text-gray-700 dark:text-gray-300">لا توجد
                                                محافظات</p>
                                            <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">ابدأ بإضافة محافظة
                                                جديدة من الزر أعلاه</p>
                                        </div>
                                    </div>
                                 </td>
                             </tr>
