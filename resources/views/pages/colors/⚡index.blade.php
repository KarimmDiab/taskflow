...
new #[Title('الوان التيشرتات')] class extends Component {
    use WithPagination;
        ...
        return Color::query()
            ...
            ->when($this->search, function ($q) {
                $q->where(function ($query) {
                    $query->where('color_name', 'like', "%{$this->search}%")->orWhere('color_hex_code', 'like', "%{$this->search}%");
                });
            })
    ...
    public function getTotalColorsProperty()
    {
        return Color::count();
    }
    ...
    {
        abort_unless(auth()->user()?->can('colors.update'), 403);

        $this->dispatch('editColor', id: $id);
    }

    public function delete($id)
    {
        abort_unless(auth()->user()?->can('colors.delete'), 403);

        Color::findOrFail($id)->delete();
        session()->flash('success', 'تم حذف اللون بنجاح');
        $this->resetPage();
    }
...
<div dir="rtl"
    ...
    <div class="mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            <div class="relative px-6 py-8">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h1
                                    ...
                                    إدارة الوان التيشرتات
                                </h1>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    منصة متكاملة لإدارة وتنظيم جميع الوان التيشرتات بكفاءة عالية
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between">
                            @php
                                $lastUpdated = Color::latest('updated_at')->value('updated_at');
                            @endphp
        ...
        {{-- Modals --}}
        @can('colors.create') <livewire:color.create /> @endcan

        @can('colors.update') <livewire:color.edit /> @endcan
                ...
                <div class="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1.5 mr-3">
                            عرض {{ $this->colors->firstItem() ?? 0 }} - {{ $this->colors->lastItem() ?? 0 }}
                            من أصل <span
                                class="font-semibold text-gray-600 dark:text-gray-300">{{ $this->colors->total() }}</span>
                            لون
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <select wire:model.live="perPage"
                            ...
                            <option value="10">10 الوان</option>
                            <option value="25">25 لون</option>
                            <option value="50">50 لون</option>
                            <option value="100">100 لون</option>
                        </select>
                    </div>
                </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50/80 dark:bg-gray-800/40 border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 transition group"
                                wire:click="sortByColumn('color_name')">
                                <div class="flex items-center gap-1">
                                    اسم اللون
                        ...
                        @forelse ($this->colors as $color)
                            <tr wire:key="color-{{ $color->id }}"
                                ...
                                <td class="px-6 py-4 text-gray-400 dark:text-gray-600 text-xs font-mono font-medium">
                                    {{ $loop->iteration + ($this->colors->currentPage() - 1) * $this->colors->perPage() }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            {{ mb_substr($color->color_name, 0, 2) }}
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end items-center gap-2">
                                        ...
                                        <button wire:click="delete({{ $color->id }})"
                                            wire:confirm="هل أنت متأكد من حذف هذا اللون؟"
