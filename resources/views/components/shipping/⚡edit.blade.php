...
new class extends Component
{
    ...
    public function editShipping($id)
    {
        ...
        $this->form->setShipping($shipping);
        Flux::modal('edit-gov')->show();
    }
    ...
    public function updateshipping()
    {
        ...
        try {
            $this->form->update();
            Flux::modal('edit-gov')->close();
            session()->flash('warning', 'تم تحديث بيانات المحافظة بنجاح');
            $this->redirectRoute('shipping', navigate: true);
        } catch (\Exception $e) {
            session()->flash('error', 'حدث خطأ أثناء تحديث المحافظة');
    ...
    public function confirmDelete($id)
    {
        $shipping = Shipping::findOrFail($id);
        $this->form->setShipping($shipping);
        Flux::modal('delete-gov')->show();
    }

    public function deleteBranch()
    {
        $this->form->shipping->delete();
        Flux::modal('delete-gov')->close();
        session()->flash('success', 'تم حذف بيانات المحافظة بنجاح');
        $this->redirectRoute('shipping', navigate: true);
    }
};
?>
...
<flux:modal name="edit-gov" class="md:w-[600px] overflow-hidden rounded-2xl" style="padding: 0;">

    <form wire:submit.prevent="updateshipping" class="relative">
...
<div class="p-6 space-y-6 bg-white dark:bg-zinc-900">
    ...
    <div>
        <flux:label class="mb-1.5 font-semibold text-gray-700 dark:text-gray-300">
            الزمن المتوقع <span class="text-red-500 dark:text-red-400">*</span>
        </flux:label>
        <flux:textarea
            placeholder="مثال: 2 - 3 أيام"
            wire:model="form.estimated_days"
            wire:dirty.class="border-amber-400 ring-2 ring-amber-400/50"
            rows="3"
