...
new #[Title('إدارة بنود المصروفات')] class extends Component {
    use WithPagination;
    ...
    {
        return ExpensesItem::query()
            ->orderBy($this->sortBy, $this->sortDirection)
            ->when($this->search, function ($q) {
                $q->where(function ($query) {
                    $query->where('expenses_name', 'like', "%{$this->search}%");
                });
            })
            ->paginate(10);
    }

    public function getTotalExpensesItemsProperty()
    {
        return ExpensesItem::count();
    }
    ...
    {
        abort_unless(auth()->user()?->can('expense_items.update'), 403);

        $this->dispatch('editExpensesItem', id: $id);
    }

    public function delete($id)
    {
        abort_unless(auth()->user()?->can('expense_items.delete'), 403);

        ExpensesItem::findOrFail($id)->delete();
...
<div dir="rtl"
    ...
    <div class="mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            <div class="space-y-1">
                <div class="flex items-center gap-3">
                    ...
                    <span
                        ...
                        {{ $this->totalExpensesItems }} إجمالي
                    </span>
                </div>
                <p class="text-sm text-gray-500 dark:text-gray-400 pr-5">
                    إدارة وتنظيم جميع بنود المصروفات بكفاءة عالية وإحصائيات دقيقة
                </p>
            </div>

            @can('expense_items.create')
            <flux:modal.trigger name="add_expenses_item">
                <button
        ...
        {{-- Modals --}}
        @can('expense_items.create') <livewire:expenses_item.create /> @endcan
        @can('expense_items.update') <livewire:expenses_item.edit /> @endcan
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-all duration-300"
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1.5 mr-2.5">
                            ...
                            من أصل <span
                                class="font-medium text-gray-600 dark:text-gray-300">{{ $this->expenses_items->total() }}</span>
                            بند
                        </p>
                    </div>
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="بحث في بنود المصروفات..."
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800/50">
                        @forelse ($this->expenses_items as $expenses_item)
                            <tr wire:key="expenses_item-{{ $expenses_item->id }}"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            {{ mb_substr($expenses_item->expenses_name, 0, 2) }}
                                        </div>
                                        <div>
                                            <p class="font-semibold text-gray-900 dark:text-white text-sm">
                                                {{ $expenses_item->expenses_name }}
                                            </p>
                                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 font-mono">
                                                كود بند المصروفات: {{ $expenses_item->id }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-0.5">
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                            {{ $expenses_item->created_at->translatedFormat('d F Y') }} </span>
                                        <span class="text-xs text-gray-400 dark:text-gray-500">
                                            {{ $expenses_item->created_at->diffForHumans() }}
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @can('expense_items.update')
                                    <button wire:click="edit({{ $expenses_item->id }})"
                                    ...
                                    @can('expense_items.delete')
                                    <button wire:click="delete({{ $expenses_item->id }})"
                                            wire:confirm="هل أنت متأكد من حذف هذا البند"
            ...
            @if ($this->expenses_items->hasPages())
                <div
